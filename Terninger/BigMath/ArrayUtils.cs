// Credit and copyright to Alexander Logger (MIT license)
...
namespace BigMath.Utils
{
    ...
    public static class ArrayUtils
    {
        ...
        {
            var ret = new byte[(int)arrays.Sum(x => x.Length)];
            int offset = 0;
        ...
        // Additional methods added by Murray Grant.

        public static Int128 Sum<T>(this IEnumerable<T> collection, Func<T, Int128> selector)
        {
        ...
        public static Int128 Min<T>(this IEnumerable<T> collection, Func<T, Int128> selector)
        {
            if (collection == null) throw new ArgumentException(nameof(collection));
            var result = Int128.MaxValue;
            foreach (T item in collection)
                result = result > selector(item) ? selector(item) : result;
            return result;
        }
        public static Int128 Max<T>(this IEnumerable<T> collection, Func<T, Int128> selector)
        {
            if (collection == null) throw new ArgumentException(nameof(collection));
            var result = Int128.MinValue;
            foreach (T item in collection)
                result = result < selector(item) ? selector(item) : result;
            return result;
        }
        ...
        // End additional methods added by Murray Grant.
    }
}